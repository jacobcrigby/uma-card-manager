...
def add_card(cards: List[UserCard], name: str, card_type: int, rarity: int) -> tuple[List[UserCard], str]:
	"""
	...
	if idx != -1:
		...
		if current_lb >= 4:
			return cards, f"Card '{name}' is already at max limit break (lb=4), ignoring"
		else:
			cards[idx]["lb"] = current_lb + 1
			return cards, f"Increased lb for '{name}' from {current_lb} to {current_lb + 1}"
	else:
