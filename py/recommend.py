...
def get_best_cards_by_type_from_tierlist(
    tierlist_path: Path, my_cards: List[EnrichedCard]
    ...
    mlb_ids = {
        c.get("id") for c in my_cards if c.get("lb") == 4 and c.get("id") is not None
    }
    ...
    for card_id, card_info in cards.items():
        ...
        if (
            not current_best
            or get_tier_value(max_tier) > get_tier_value(current_best[4])
            or (
                get_tier_value(max_tier) == get_tier_value(current_best[4])
                and max_score > current_best[3]
            )
        ):
            card_name = card_dict.get("name", "Unknown")
            if not isinstance(card_name, str):
                card_name = "Unknown"
            best_by_type[card_type] = (
                card_id_int,
                card_name,
                card_type,
                max_score,
...
def find_best_card_in_tierlist(
    tierlist_path: Path, my_cards: List[EnrichedCard]
...
def format_card_display(
    ...
    if rarity is not None:
        rarity_display = (
            "SSR" if rarity == 3 else ("SR" if rarity == 2 else str(rarity))
        )
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.counts:
        if len(args.counts) > 6:
            print(
                "error: too many positional arguments (max 6: speed stamina power guts wit friend)",
                file=sys.stderr,
            )
            return 1
    ...
    if not args.no_support and total_cards < 6:
        try:
            ...
            if best_support:
                ...
                print(
                    f"Found best support card to borrow: {best_support[1]} ({TYPE_NAMES.get(best_support[2])}) (Tier: {best_support[4]}, Score: {best_support[3]})\n"
                )
    ...
    if not support_card:
        ...
        if current_total < 6:
            ...
            if num_to_add > 0 and remaining_cards:
                print(
                    f"Filled {min(num_to_add, len(remaining_cards))} remaining slot(s) with best available cards\n"
                )
...
def run_best_cards(args: argparse.Namespace) -> int:
    ...
    if not args.no_support:
        try:
            best_id, best_name, best_type, best_score, best_tier = (
                find_best_card_in_tierlist(args.tierlist, my_cards)
            )
            support_card = (best_id, best_name, best_type, best_score)
            exclude_id = best_id
            print(
                f"Found best support card to borrow: {best_name} ({TYPE_NAMES.get(best_type)}) (Tier: {best_tier}, Score: {best_score})\n"
            )
        except Exception as e:
            print(f"warning: could not find support card: {e}", file=sys.stderr)
